...
一個基於深度學習的瑜伽姿勢識別系統，實現高達95.7%準確率的高效動作識別，並提供即時的 AI 矯正指導與訓練追蹤！
...
- Problem Framing：將「做 AI 瑜珈教練」拆成「以一般攝影機完成低成本姿勢分類＋角度提示」的 PoC。
- MVP 範圍界定：測試多種情境（單人 / 雙人、距離、背景），最後收斂為「單人＋背景簡單＋5 類姿勢」的可用範圍。
- 競品掃描：健身鏡 / 智慧瑜珈墊 / 穿戴裝置多依賴專用硬體，本專案選擇「免穿戴、裝置門檻低」作為差異點。
- 未來延伸方向：擴充姿勢類別、加入練習日誌與成就系統，並與瑜珈教室 / 社群串接。
> 產品背景、問題拆解與使用邊界整理：請見 [`docs/product_notes.md`](docs/product_notes.md)
...
flowchart TD
    subgraph Feature Extraction
        A[dataset.zip] -->|extract_landmarks.py| B[landmarks_results.zip]
        B -->|calculate_features.py| C[yoga_pose_features/]
        C -->|normalize_data.py| D[yoga_features_norm/]
    end
...
- 使用 MediaPipe 提取人體關鍵點，將關鍵點轉換為姿勢的幾何特徵（如角度與長度）。

- 採用 LSTM + Attention 和 Transformer Encoder 模型，進行時序分析和姿勢分類。

2. 即時反饋

利用 Gradio 實現即時影像捕捉和姿勢識別，並為使用者提供姿勢矯正，姿勢標準時有正面音效回饋。

3. 數據儲存與分析

使用 MySQL 儲存每次練習的紀錄，並提供用戶界面進行查詢和分析。

---
## 使用者界面

透過 Gradio 提供直觀的界面，並支援即時姿勢反饋和結果儲存至資料庫。介面簡單易用，適合瑜伽學習者進行自我檢測。
